...
function checkTimeBasedTheme() {
  ...
  if (override) {
      ...
      const icon = el("themeIcon"); // kept for failsafe
      // Update label in settings
      if(themeLabel) themeLabel.textContent = override === "light" ? "Light Mode" : "Dark Mode";
      return;
  }
  ...
  if(themeLabel) themeLabel.textContent = isDay ? "Auto (Day)" : "Auto (Night)";
}
...
function showToast(message, type = "success") {
  ...
  if (typeof vibrate === "function") vibrate(type === "error" ? [30,30] : [20]);
  setTimeout(() => {
...
function checkAppVersion() {
  ...
  const display = el("settingsVersionDisplay");
...
function computeDerivedFields(loan) {
  ...
  const daysOverdue = (today > dueDate && status !== "PAID") ? Math.floor((today - dueDate) / (1000 * 60 * 60 * 24)) : 0;
  loan.rate = rate;
...
window.openReceipt = function(loanId) {
  ...
  printWindow.document.write(`<html><head><style>body { font-family: monospace; padding: 20px; text-align: center; } .header { font-size: 1.2em; font-weight: bold; margin-bottom: 10px; border-bottom: 2px dashed #000; padding-bottom: 10px; } .row { display: flex; justify-content: space-between; margin: 5px 0; } .footer { margin-top: 20px; border-top: 1px solid #000; padding-top: 10px; font-size: 0.8em; }</style></head><body><div class="header">STALLZ LOANS<br>OFFICIAL RECEIPT</div><div class="row"><span>Date:</span> <span>${new Date().toLocaleDateString()}</span></div><div class="row"><span>Loan ID:</span> <span>#${loan.id}</span></div><div class="row"><span>Client:</span> <span>${loan.clientName}</span></div><br><div class="row"><span>Principal:</span> <span>${formatMoney(loan.amount)}</span></div><div class="row"><span>Plan:</span> <span>${loan.plan}</span></div><div class="row"><span>Total Due:</span> <span>${formatMoney(loan.totalDue)}</span></div><div class="row"><span>Paid So Far:</span> <span>${formatMoney(loan.paid)}</span></div><br><div class="row" style="font-weight:bold; font-size:1.1em;"><span>BALANCE:</span> <span>${formatMoney(loan.balance)}</span></div><div class="footer">Generated by: ${state.user ? state.user.email : 'System'}<br>Thank you for your business.</div><script>window.print();</script></body></html>`);
...
function vibrate(pattern = [15]) { if (typeof navigator !== "undefined" && navigator.vibrate) navigator.vibrate(pattern); }
...
function setupMobileUX() {
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault();
    deferredPrompt = e;
    const btn = el("settingsInstallBtn");
    if (btn) {
      // Show install button INSIDE settings modal
...
window.closeAllModals = function(resetNav = true) {
    ['monthlyModal', 'clientsModal', 'adminsModal', 'settingsModal'].forEach(id => {
...
// ==========================================
// THEME TOGGLE LOGIC (Updated for Settings)
// ==========================================
function toggleTheme() {
    ...
    // Update label text inside modal if it exists
...
function init() {
  ...
  el("settingsBtn")?.addEventListener("click", () => {
      // When opening settings, make sure theme label is correct
      checkTimeBasedTheme();
      openPopup("settingsModal");
  });
  el("settingsThemeBtn")?.addEventListener("click", toggleTheme);
  el("settingsLogoutBtn")?.addEventListener("click", () => {
      if(confirm("Are you sure you want to log out?")) handleLogout();
  });

  el("openLoanModalBtn")?.addEventListener("click", () => { if (typeof vibrate === "function") vibrate([10]); wizardStep=0; wizardDraft={}; updateWizard(); el("loanModal").classList.remove("modal-hidden"); });
  el("modalCloseBtn")?.addEventListener("click", () => el("loanModal").classList.add("modal-hidden"));
  el("modalNextBtn")?.addEventListener("click", () => { if (typeof vibrate === "function") vibrate([10]); handleWizardNext(); });
  el("modalBackBtn")?.addEventListener("click", () => { if (typeof vibrate === "function") vibrate([10]); handleWizardBack(); });
