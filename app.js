...
function toggleSidebar(show) {
    const elSidebar = el("appSidebar");
    const elBackdrop = el("sidebarBackdrop");
    if(show) {
        elSidebar.classList.add("active");
        elBackdrop.classList.add("active");
        vibrate([10]);
        // Update badges when opening
...
function toggleSound() {
    ...
    localStorage.setItem("stallz_sound_enabled", !current);
    updateSettingsBadges();
    if (!current) vibrate([30]); // Vibrate to confirm ON
}
...
// ... inside init() ...
  ...
  // 2. SIDEBAR OPTION LISTENERS
  el("menuThemeBtn")?.addEventListener("click", () => {
      toggleTheme();
      updateSettingsBadges();
  });
  ...
  el("menuExportBtn")?.addEventListener("click", () => {
      toggleSidebar(false);
      // Trigger your existing export logic here
      const exportBtn = el("exportBtn"); // Or copy the export logic code here
  ...
  el("menuLogoutBtn")?.addEventListener("click", () => {
      toggleSidebar(false);
      setTimeout(() => {
          if(confirm("Log out of admin panel?")) handleLogout();
      }, 200);
  });
