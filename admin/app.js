...
async function sendCustomAlertToClient(clientUid, title, message) {
    try {
        ...
        // Write once — Cloud Functions will:
        // 1) mirror it into stallzShared_v1/notifications/users/{uid}
        // 2) deliver a real OS-level push via FCM (even when the app is closed)
        await firebase.database().ref(`clients/${clientUid}/notifications/${notifId}`).set(payload);

        showToast("Alert sent ✅ (will deliver via push)", "success");
    } catch (error) {
...
// ==========================================================================
// ADMIN PUSH NOTIFICATIONS (FCM: Foreground + Background via Service Worker)
// ==========================================================================
async function initAdminPushNotifications(forcePrompt = false) {
    ...
    try {
        // 1) Ask permission (only on user action)
        if (Notification.permission !== 'granted') {
            if (!forcePrompt) return false;
            const perm = await Notification.requestPermission();
            if (perm !== 'granted') {
                showToast("Notifications are off. Enable them in browser settings to receive alerts.", "error");
                updateAdminPushPermissionUI();
                return false;
            }
        }
        ...
        if (!token) {
            showToast("Couldn't enable notifications on this device. Please try again.", "error");
            return false;
        }

        // Save token for auth handoff
        const prevActive = localStorage.getItem("stallz_active_fcm_token");
        if (prevActive !== token) localStorage.setItem("stallz_pending_fcm_token", token);
        localStorage.setItem("stallz_active_fcm_token", token);

        // If logged in already, sync immediately
        try {
            const user = firebase.auth().currentUser;
            if (user && window.StallzAuth?.syncPendingFCMToken) {
                await window.StallzAuth.syncPendingFCMToken(user.uid, "admin");
            }
        } catch (_) {}

        // Foreground handler (avoid binding twice)
        if (!window.__STALLZ_ADMIN_FOREGROUND_PUSH_BOUND) {
            window.__STALLZ_ADMIN_FOREGROUND_PUSH_BOUND = true;
            messaging.onMessage((payload) => {
                console.log('[Admin Foreground] Push received: ', payload);

                try { if (typeof vibrate === "function") vibrate([20, 40, 20]); } catch(_) {}
                ...
                // refresh dropdown list if present
        ...
        if (forcePrompt) showToast("✅ Notifications enabled on this device.", "success");

        return true;

    } catch (err) {
        console.error("Failed to initialize admin push:", err);
        showToast("Push setup failed. Please try again.", "error");
        return false;
    }
}

window.handleEnableAdminPushClick = async function() {
    try { if (typeof vibrate === "function") vibrate([10]); } catch(_) {}
    await initAdminPushNotifications(true);
};

window.updateAdminPushPermissionUI = function() {
    try {
        const badge = document.getElementById("adminPushBadge");
        if (!badge) return;
        const perm = (typeof Notification !== "undefined") ? Notification.permission : "unsupported";

        badge.classList.remove("paid", "overdue", "active");
        if (perm === "granted") {
...
async function ensureAdminAccess() {
  ...
  try {
    ...
    // --- ADMIN PUSH NOTIFICATION AUTO-START ---
    try {
      if ('Notification' in window && Notification.permission === 'granted') {
        if (typeof initAdminPushNotifications === 'function') initAdminPushNotifications(false);
      }
    } catch(_) {}
    // -----------------------------------------
